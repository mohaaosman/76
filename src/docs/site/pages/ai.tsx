        ...
        <Row overlap split="main">
          <Card>
            ...
            <div className="site-prose">
              <p>
                <RichText text="<b>1 · The registry.</b> Every component ships as a shadcn-compatible registry item at <code>/r/&lt;slug&gt;.json</code> — description, feature tags, category, file contents, and registry dependencies. Any AI agent that can run <code>npx shadcn add &lt;url&gt;</code> installs the component with correct imports; it never has to paste code from memory." />
              </p>
              <p>
                <RichText text="<b>2 · Markdown routes.</b> Each doc page is generated from pure data, so the same source renders this site and emits <code>/llms/&lt;slug&gt;.md</code>. The 'Copy page for AI' button on every component page yields the identical markdown. <code>/llms.txt</code> is the index." />
              </p>
              <p>
                <RichText text="<b>3 · MCP (later).</b> Once the registry has volume, an MCP server wraps it with search_components / get_component tools, the Untitled UI pattern. The metadata needed for semantic search — descriptions, tags, categories — already exists in every registry item, so nothing needs re-authoring." />
              </p>
            </div>
          </Card>
          <Card>
            ...
            <div className="site-prose">
              <p>
                <RichText text="Every component page ends in an FAQ that restates the API in natural language — the colors available, how to disable, how to make full-width. Humans skim it; retrieval systems and LLMs match user questions against it almost verbatim. It is documentation and embedding fodder in one." />
              </p>
              <p>
                <RichText text="The Don't lists serve AI collaborators the same way: they are the Slop Firewall in prose, so an assistant generating 76° UI knows a filled status pill or a donut chart is a <b>defect</b>, not a stylistic choice." />
              </p>
            </div>
          </Card>
        </Row>
