...
export const templates: Composition[] = [
  ...
  {
    slug: 'invoice-checkout',
    kind: 'template',
    name: 'Invoice',
    seed: 'cobalt',
    tagline: 'A document a business sends and a customer pays — and it prints.',
    description: [
      'The invoice as a <b>document of record</b>: parties and terms in a <b>DescriptionList</b>, the lines in a <b>DataTable</b> with <code>leadHold</code> and a real <code>&lt;tfoot&gt;</code>, and the charges that belong to the document rather than to a column — discount, tax, deposit — in a <b>SumList</b> (B55) beside the single primary that names the act: "Pay $21,888.00".',
      'It states its parent purchase order as a <b>fact</b> in its own facts list rather than as a trail of chevrons above the title (F13). It is overdue, so the condition is a <b>Banner</b> where it happened, not a toast. And it is built to be printed: the printed surface shipped in 0.6, the browser repeats the table head on every page, and the remittance facts are on the sheet rather than behind a link.',
      'Every figure reconciles by hand: 12 × $980 + 40 × $160 + 24 × $145 + 2 × $1,200 + 4 × $390 = $25,600; less 5% = $24,320; VAT 20% = $4,864; total $29,184; less the 25% deposit of $7,296 = <b>$21,888</b>, which is the figure on the button. Issued 12 JUN on Net 30 is due 12 JUL, thirteen days before the 25 JUL this screen states.',
    ],
    tags: ['invoice', 'billing', 'finance', 'document', 'print', 'sum-list', 'cobalt'],
    file: 'templates/invoice-checkout.tsx',
    deps: ['band', 'sheet', 'card', 'description-list', 'data-table', 'sum-list', 'timeline', 'banner', 'status-word', 'button'],
  },
  {
    ...
    name: 'Checkout',
    seed: 'signal',
    tagline: 'A card payment in the shape people expect, refusing what 76° refuses.',
    description: [
      'Two columns on the wall: a <b>Stepper</b> (B39) over a B11 form with <b>ErrorSummary</b> (B52) at its head and every field also stating its own error, beside the order summary — a <b>DataTable</b> of the items and a <b>SumList</b> of the charges. The step lives in the URL, so back, reload and a pasted link all behave; a reloaded confirmation with no payment behind it falls back to the form rather than printing an unsubstantiated receipt.',
      '<b>What it refuses is the point.</b> No card-brand logos (B26 allows one <code>currentColor</code> path, never brand hexes — the accepted brands are named in the field hint). No padlock, no trust badge, no "100% secure" (A3). No countdown and no scarcity (F12, A3). No masked self-formatting card input fighting the browser: ordinary Fields with <code>cc-number</code>, <code>cc-exp</code> and <code>cc-csc</code>, correct <code>inputMode</code>, and errors wired through <code>aria-describedby</code>.',
      'F14 governs the colour: the total is <b>ink</b>, the button is the only seed rectangle on the screen. 6 × $84 + 2 × $312 + 8 × $46.50 = $1,500; + $95 shipping = $1,595; VAT 20% = $319; <b>$1,914</b> — the figure on the button.',
    ],
    tags: ['checkout', 'payment', 'card', 'ecommerce', 'stepper', 'sum-list', 'signal'],
    file: 'templates/payment-checkout.tsx',
    deps: ['band', 'sheet', 'card', 'data-table', 'sum-list', 'stepper', 'error-summary', 'field', 'banner', 'button'],
  },
  {
    slug: 'erp-purchase-order',
    kind: 'template',
    name: 'Purchase order',
    seed: 'cobalt',
    tagline: 'One operational document — its facts, its lines, its history, its verbs.',
    description: [
      'The record page. Thirteen templates shipped before this one and none of them drew a record: every screen was a list, an overview or a decision. A management system is mostly neither — it is a person inside ONE named document.',
      'It composes three things the Book mandated and no screen had ever put together. <b>B28</b> defers record editing to "a Drawer with a form" — here the <b>DescriptionList</b> states and the <b>Drawer</b> (B21) edits, without the page ever navigating away. <b>B11</b>\'s top-of-form error summary sits at the head of that form as <b>ErrorSummary</b> (B52), indexing what failed while each field still states its own error. And <b>B7</b>\'s v0.6 <code>totals</code> and <code>leadHold</code> close the document in a real <code>&lt;tfoot&gt;</code> and hold the line identity while the wide table scrolls under it.',
      'Every figure reconciles: line extensions are quantity × unit cost, the subtotal is their sum, VAT is 20% of it, and the total is the two added. Ship Gate 12 is checkable on this screen by hand, which is the only way a mockup of a financial document is honest.',
    ],
    tags: ['erp', 'record', 'purchase-order', 'document', 'drawer', 'totals', 'cobalt'],
    file: 'templates/erp-purchase-order.tsx',
    deps: ['band', 'sheet', 'card', 'description-list', 'data-table', 'timeline', 'banner', 'drawer', 'error-summary', 'field', 'number-field', 'status-word', 'badge', 'menu', 'button'],
  },
  {
    slug: 'crm-deal-board',
    kind: 'template',
    name: 'Deal board',
    seed: 'verdigris',
    tagline: 'The pipeline as a board — four stage cards, and a deal that moves by a named verb.',
    description: [
      'F2 refuses the Kanban board <i>widget</i> and answers it with "a template (the CRM already has one)". It did not: <code>crm-pipeline</code> is a dashboard of stats, a stage meter and a trend. F2 has been citing a screen that was never drawn. This is that screen, and <b>F2 stands completely unamended</b> — there is no board component here, only four stage cards, each holding a B7 <b>DataTable</b> of the deals in it.',
      'A deal moves by a <b>named verb, never by drag</b>. F10\'s reasoning reaches this screen intact: a drag target has no keyboard equivalent, no accessible name, and no state a screen reader can report mid-gesture. Opening a deal gives it a Select and one primary that names the act — which works from the keyboard and states where the deal is going before it goes.',
      'Four columns, because B2\'s canonical stats split IS four and already knows how to become 2×2 below 1000px and single file below 560px. A board that scrolls sideways is the thing A2 refuses on every nav-like row, so "Stalled" is a <b>condition</b> stated in a Banner above the board — not a fifth column deals go to die in.',
    ],
    tags: ['crm', 'pipeline', 'board', 'kanban', 'deals', 'drawer', 'verdigris'],
    file: 'templates/crm-deal-board.tsx',
    deps: ['band', 'sheet', 'card', 'data-table', 'description-list', 'drawer', 'banner', 'field', 'search-field', 'status-word', 'avatar', 'button'],
  },
  {
    slug: 'erp-dashboard',
    kind: 'template',
